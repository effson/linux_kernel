...
struct inode {
	...
	struct address_space	*i_mapping; /*该文件的地址空间 (struct address_space) 的指针。address_space 管理文件的页缓存（Page Cache）
						以及与块设备的映射关系。i_mapping->a_ops 提供页缓存操作函数（readpage, writepage, set_page_dirty 等）*/ 
