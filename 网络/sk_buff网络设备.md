...
### 3.NAPI
<mark>NAPI（New API）</mark>是 Linux 内核中用于 提高网络收包效率的一种机制，它的全称是 New API for packet processing，主要解决传统<mark>中断驱动收包模式</mark>下的性能瓶颈问题。
#### 3.1为什么需要 NAPI？
传统网络驱动使用中断方式接收数据，每收到一个数据包就触发一次中断。但在高并发网络环境下，频繁中断会造成：
- CPU被中断风暴打爆（Interrupt Storm）
- 上下文切换开销大
- 效率低
#### 3.2 NAPI 的核心思想
收到数据包后，先触发一次中断，然后关闭中断，改为轮询（polling）方式处理包，直到包处理完，再打开中断。它是一种“中断 + 轮询”的混合模式（interrupt mitigation）：
- 初始由中断触发（通知有数据）
- 然后进入轮询模式处理多个包（减少中断次数）
- 如果队列空了，再重新开启中断
#### 3.3 轮询方式
- 网卡收到第一个包后，触发一次中断
- CPU 响应中断后，关闭后续中断
