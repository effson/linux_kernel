...
<img width="1124" height="463" alt="image" src="https://github.com/user-attachments/assets/2df2dc2e-93e4-470e-be54-d5313e493e81" />
...
```c
struct socket {
	socket_state		state; // 套接字状态：SS_CONNECTED, SS_DISCONNECTING 等
	short			type;
	unsigned long		flags;
...
struct socket {
	...
	struct file		*file;
	struct sock		*sk; // 一个与协议无关的结构，位于网络层
	const struct proto_ops	*ops; /* Might change with IPV6_ADDRFORM or MPTCP. */
	struct socket_wq	wq;
};
```
