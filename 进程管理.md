...
struct task_struct {
    // ...
    int prio; //进程的动态优先级（调度器实际使用的优先级）。
    int static_prio; //进程的静态优先级（由用户或系统设定，nice 值映射而来）。
    int normal_prio; //基于 static_prio 和调度策略计算出的归一化优先级。
    unsigned int rt_priority; //实时进程的优先级（0-99，值越大优先级越高）。 
    // ...
};
```
## 2.4 内存描述符
...
```
real_parent（真实父进程）指向实际创建该进程的原始父进程。例如，在终端中运行程序时，bash 调用 fork() 创建子进程，子进程的 real_parent 指向 bash 的 task_struct<br>
parent（信号接收父进程）指向接收该进程信号（如 SIGCHLD）的父进程。默认情况下 parent = real_parent，但当进程被调试器（如 gdb）跟踪时，parent 会指向调试器进程<br>
```
核心组成部分：
